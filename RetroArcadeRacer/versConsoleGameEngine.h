...
the application to close.
	...
	int main()
	{
		// Use versConsoleGameEngine derived app
...
public:
	...
	versSprite( int w, int h )
	{
		Create( w, h );
	}

	versSprite( std::wstring sFile )
	{
		if (!Load( sFile ))
			Create( 8, 8 );
	}
...
class versConsoleGameEngine
{
public:
	versConsoleGameEngine()
	{
		m_nScreenWidth = 80;
		m_nScreenHeight = 30;
	...
	void DrawSprite( int x, int y, versSprite * sprite )
	{
		if (sprite == nullptr)
			return;
	...
	void DrawPartialSprite( int x, int y, versSprite * sprite, int ox, int oy, int w, int h )
	{
		if (sprite == nullptr)
			return;
...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread( &versConsoleGameEngine::GameThread, this );
...
protected: // Audio Engine =====================================================================
	...
	public:
		...
		versAudioSample( std::wstring sWavFile )
		{
			...
			// Just check if wave format is compatible with versCGE
	...
	std::vector<versAudioSample> vecAudioSamples;
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512 )
	{
		...
		m_AudioThread = std::thread( &versConsoleGameEngine::AudioThread, this );
	...
	{
		((versConsoleGameEngine*)dwInstance)->waveOutProc( hWaveOut, uMsg, dwParam1, dwParam2 );
	}
...
std::atomic<bool> versConsoleGameEngine::m_bAtomActive( false );
std::condition_variable versConsoleGameEngine::m_cvGameFinished;
std::mutex versConsoleGameEngine::m_muxGame;