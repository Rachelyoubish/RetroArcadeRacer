...
#include "versConsoleGameEngine.h"

class versFormula : public versConsoleGameEngine {
public:
	versFormula() {
		m_sAppName = L"Formula Vers";
	}
...
protected:
	...
	virtual bool OnUserUpdate( float fElapsedTime ) {
		...
		for (int x = 0; x < ScreenWidth(); x++) {
			int nHillHeight = (int)(fabs( sinf( x * 0.0f + fTrackCurvature ) * 16.0f ));
		...
		for (int y = 0; y < ScreenHeight() / 2; y++)
			for (int x = 0; x < ScreenWidth(); x++) {
				...
				float fMiddlePoint = 0.5 + fCurvature * powf( (1.0f - fPerspective), 3 );
